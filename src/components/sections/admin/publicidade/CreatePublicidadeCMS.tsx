...
export default function CreatePublicidadeCMS() {
  ...
  const handleCriarPublicidade = async () => {
    if (!titulo || !conteudo) {
    ...
    try {
      ...
      if (arquivoParaEnvio) {
        formData.append("file", arquivoParaEnvio);
      }

      await publicidadeService.criar(formData);
      navigate(-1);

    } catch (error) {
      console.error("Erro ao salvar:", error);
      alert("Ocorreu um erro ao salvar a publicidade no servidor.");
    } finally {
      setSalvando(false);
    }
  };
  ...
  return (
    <div className="min-h-screen p-8 text-muted-foreground animate-in fade-in duration-500">
      ...
      <header className="mb-8">
        <h1 className="text-2xl font-bold text-secondary">Criar nova publicidade</h1>
        <p className="text-sm text-gray-500">Preencha os campos abaixo para configurar e publicar a publicidade.</p>
      </header>
      ...
      <footer className="mt-12 flex justify-end items-center gap-4">
        ...
        <Button 
          type="button"
          onClick={handleCriarPublicidade}
          disabled={salvando}
        ...
        >
          <Printer size={18} />
          {salvando ? "Salvando publicidade..." : "Criar publicidade"}
        </Button>
      </footer>
    </div>
  );
