...
export default function FinanceiroDashboard() {
  const data = mockDashboard.financeiro;
    ...
    <div className="p-6 space-y-8 bg-white min-h-screen font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
        {[
          ...
          <Card key={i} className="rounded-2xl border border-gray-100 bg-white min-h-[150px] flex items-center">
        ...
        <Card className="lg:col-span-6 rounded-[40px] border border-gray-50 p-8 bg-white">
          ...
          <div className="h-64 px-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={historicoRecente} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
                ...
                <Bar dataKey="receitaRealizada" radius={[10, 10, 10, 10]} barSize={45}>
                  {historicoRecente.map((_, index) => (
                    <Cell key={`cell-${index}`} fill={index === historicoRecente.length - 1 ? '#002749' : '#3498DB'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <div className="flex gap-6 mt-4">
              {porMetodoPagamento.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-bold text-black">{item.metodo}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <Card className="rounded-[30px] border border-gray-100 p-8 bg-white flex flex-col justify-between">
          <div>
            ...
            <h2 className="text-4xl font-black text-[#333333]">
              {formatCurrency(data.previsaoCaixa30Dias.valorTotal)}
            </h2>
          </div>
          <div className="flex justify-end items-center mt-6">
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-black text-[#333333]">{data.previsaoCaixa30Dias.quantidadeParcelas}</span>
        ...
        <Card className="rounded-[30px] border border-gray-100 p-8 bg-white flex flex-col justify-between">
          ...
          <div className="flex justify-end items-center mt-6">
            <div className="flex items-center gap-2">
              ...
              />
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-black text-[#333333]">{data.inadimplencia.quantidadePagamentos}</span>
