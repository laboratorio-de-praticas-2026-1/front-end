...
const ClienteItem = ({ nome, info }: { nome: string; info: string | number }) => (
...
export default function ClientesDashboard() {
  const clientes = mockDashboard.clientes;

  const topPorVolume = clientes.topPorVolume.map(c => ({
    usuarioId: c.usuarioId,
    nome: c.nome,
    totalSolicitacoes: c.totalSolicitacoes,
  }));

  const topPorValorPago = clientes.topPorValorPago.map((c: any) => ({
    usuarioId: c.usuarioId,
    nome: c.nome,
    valorPago: c.valorPago ?? 0,
  }));

  const comParcelasEmAtraso = clientes.comParcelasEmAtraso.map(c => ({
    usuarioId: c.usuarioId,
    nome: c.nome,
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-gray-200 px-12">
              {comParcelasEmAtraso.length > 0 ? (
                ...
                <p className="text-sm text-gray-400 text-center py-8">Nenhuma parcela em atraso</p>
              )}
            </div>
