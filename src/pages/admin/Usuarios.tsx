...
import { usuarioService, type UsuarioCms } from '@/services/usuarioService';
...
const mapNivel = (nivel?: string | null): User['nivel'] => {
...
const normalizeUsuario = (usuario: UsuarioCms): User => ({
    id: Number(usuario.id),
    nome: usuario.nome || 'Sem nome',
    email: usuario.email || '-',
    nivel: mapNivel(usuario.nivel),
    dataCadastro: usuario.dataCadastro ? new Date(usuario.dataCadastro) : new Date(),
});
...
export default function Usuarios() {
    ...
    useEffect(() => {
        const delay = setTimeout(async () => {
            ...
            try {
                const dados = await usuarioService.buscarPorTermo(searchText);
                setUsers(dados.map(normalizeUsuario));
            } catch (error) {
